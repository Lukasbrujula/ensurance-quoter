...
### Tier 5 — Google Calendar Integration
| Task | Description | Est. | Depends On |
|------|-------------|------|------------|
| **T10.10a** | Google Calendar OAuth: service layer, token storage, connect/disconnect UI | 1.5 hrs | None (but dev must set up Google Cloud Console first) |
| **T10.10b** | Calendar view on dashboard + sync hooks into follow-up/AI callback flows | 1.5 hrs | T10.10a + T10.4 (dashboard) |
...
Tier 5 (sequential):
T10.10a (Google OAuth + service layer) -> T10.10b (calendar view + sync hooks)
Note: T10.10b also depends on T10.4 (dashboard page must exist)
```
